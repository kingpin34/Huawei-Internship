...
def enter_move(board):
    ...
    while not ok:
        ...
        ok=len(move)==1 and move>="1" and move<="9"#checks if users input is valid.
        if not ok:
            print("you have entered a bad Move")
            continue
        move=int(move)-1 #cell's number form 0 to 8
        row=move // 3 #cells row 
        col=move % 3  #cells column
        sign=board[row][col]  #checks the selected square
        ok=sign not in["O","X"] 
        if not ok:
...
def victory_for(board,sgn):
    if sgn == "X":
        ...
        who=None 
    cross1 = cross2 = True #checks diagonals          
    for rc in range(3):
